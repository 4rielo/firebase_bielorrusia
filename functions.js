...
updateBtn.addEventListener('click', (e) => {
    ...
    const updates ={};
    if (age.value){
        updates['/users/' + userId.value + '/age'] = age.value;  
    }
    
    if (firstName.value){
        updates['/users/' + userId.value + '/first_name'] = firstName.value;  
    }
    
    if (lastName.value){
        updates['/users/' + userId.value + '/last_name'] = lastName.value;  
    }
...
removeBtn.addEventListener('click', e=> {
   ...
   rootRef.child(userId.value).remove().then(() => {
        windows.alert('user removed from database !');
   })
   .catch(error => {
       console.error(error);
   }); 
...
//rootRef.orderByChlid('age').limitToFirst(2).on('value', snapshot => {
//    console.log(snapshot.val());
//});


// startAt(), 
//rootRef.orderByChlid('first_name').equalTo('Ramiro').on('value', snapshot => {
//    console.log(snapshot.val());
//});